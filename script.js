$(document).ready(function () {
  ...
  let provider = ["Netflix", "Hulu", "AmazonPrimeVideo", "GooglePlay", "iTunes"];
  let offerType = "";
  ...
  for (i = 0; i < provider.length; i++) {
    ...
    let settings = {
      "async": true,
      "crossDomain": true,
      "url": "https://ivaee-internet-video-archive-entertainment-v1.p.rapidapi.com/entertainment/search/?PersonNames=" + Actor + "&Title=" + Title + "&ProgramTypes=Show&ProgramTypes=Movie&Providers=" + provider[i],
      "method": "GET",
      // error: checkContent(),
      "headers": {
    ...
    $.ajax(settings).done(function (response) {
      ...
      if (service === "Netflix" || service === "Hulu" || service === "AmazonPrimeVideo") {
      ...
      console.log("The are content provider requires:", offerType, "for their service", service, "has this much content matching your search available: ", response.Hits.length);
